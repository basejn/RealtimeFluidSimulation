...
uniform float surfTensTresh=0.20;
...
float laplacian_W_poly6(float r){	
	...
	int CellIndexNeighbour(vec3 point){
		...
		tmp = (int(((point.z+offset)/cellSize))*gridSide*gridSide);
		cellIndex = (tmp>=0)?((tmp<=gridSide*gridSide*(gridSide-1))?(tmp):(-1000)):-1000;
		tmp = (int(((point.y+offset)/cellSize))*gridSide);
		cellIndex += (tmp>=0)?((tmp<=gridSide*(gridSide-1))?(tmp):(-1000)):-1000;
	    tmp = (int(((point.x+offset)/cellSize)));
		cellIndex += (tmp>=0)?((tmp<=(gridSide-1))?(tmp):(-1000)):-1000;		
...
//	 m = velocity_mass.w;     // m is the mass of our vertex
 //    v = velocity_mass.xyz;             // u is the initial velocity
	...
	*/
	int curCellIndex;
	const vec3[27] offsets=vec3[27](vec3(cellSize,cellSize,0),  vec3(cellSize,cellSize,cellSize),  vec3(cellSize,cellSize,-cellSize),
									...
									vec3(0,0,0),  vec3(0,0,cellSize),  vec3(0,0,-cellSize),
									vec3(0,-cellSize,0), vec3(0,-cellSize,cellSize), vec3(0,-cellSize,-cellSize),
									vec3(-cellSize,cellSize,0), vec3(-cellSize,cellSize,cellSize), vec3(-cellSize,cellSize,-cellSize),
									vec3(-cellSize,0,0), vec3(-cellSize,0,cellSize), vec3(-cellSize,0,-cellSize),
									vec3(-cellSize,-cellSize,0),vec3(-cellSize,-cellSize,cellSize),vec3(-cellSize,-cellSize,-cellSize));
	...
	curCellIndex = CellIndexNeighbour(position+offsets[i]);
		if((curCellIndex>=0))
		{
			#if (OPTIM_STRUCT==1)
