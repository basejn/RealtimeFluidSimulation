...
float intersect_ray_sphere(vec3 orig,vec3 dir,vec4 sphere)
{
...
vec4 calcLight(vec3 N,float t){
    ...
    vec3 diffuse = max(dot(N, L), 0.0) *tmpColor.xyz;
    vec3 specular = pow(max(dot(R, V), 0.0), 4) * vec3(0.7,0.7,0.8); 
	vec3 ambient =  max(dot(N, V), 0.0) *tmpColor.xyz*0.2;
...
float intersect_ray_sphere1(vec3 origin,vec3 direction,vec4 sphere)
{
