...
uniform float rest_density=0.1;
uniform float eta=1.20;
uniform float sigma=1.0;
uniform float dens_K=50.5;
uniform float surfTensTresh=0.10;
uniform float deltaT=0.2;
const float h=5;
uniform float GRID_VOLUME_SIDE=10;
const int gridSide=3;
uniform float cellSize=20.0/3.0;//GRID_VOLUME_SIDE/gridSide
...
float laplacian_W_poly6(float r){	
	...
	vec3 npos = texelFetch(tex_position,i).xyz;			
			...
			if(dst<=h){
			dstV=normalize(dstV);
			if(dst==0){return;	dstV=vec3(1,1,1);dst=1;}
...
const float wall_mass=100;	
			...
			if(abs(dst)<=h){			
			if(dst==0){	return;dstV=vec3(1,1,1);dst=1;}
			vec2 n_density_pressure = vec2(1,40);
...
/*
 ...
 //* 
	...
	forEveryWall(vec4(0,1,0,GRID_VOLUME_SIDE*1.3));
	forEveryWall(vec4(1,0,0,GRID_VOLUME_SIDE*1.3));
	forEveryWall(vec4(-1,0,0,GRID_VOLUME_SIDE*1.3));
	forEveryWall(vec4(0,0,1,GRID_VOLUME_SIDE*1.3));
	forEveryWall(vec4(0,0,-1,GRID_VOLUME_SIDE*1.3));
