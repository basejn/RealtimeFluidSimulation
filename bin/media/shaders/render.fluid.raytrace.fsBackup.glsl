...
uniform vec4 sphereColor = vec4(0,1,0,1);
...
float reflSphereT = intersect_ray_sphere2(fs_in.ray_origin+rayDir*curt,reflDir,sphere1);

vec4 firstReflColor;
if(reflSphereT>0) firstReflColor = sphereColor;
...
if(secRefrSphereT>0)refrOutRayColor = sphereColor;
...
vec3 reflWaterOrig;
vec3 reflWaterNorm;
float reflWaterT =  findIsosurface( 0, 20,true,(fs_in.ray_origin + rayDir*firstSphereT),reflDir,reflWaterOrig,reflWaterNorm);
if(reflWaterT>0){
